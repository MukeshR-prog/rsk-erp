...
export default function ProductCategoriesPage() {
  ...
  const [deactivatingCategory, setDeactivatingCategory] = useState<CategoryData | null>(null);
  ...
  const loadCategories = () => {
    startTransition(async () => {
      ...
      if (res.success && res.data) {
        setCategories(res.data as CategoryData[]);
  ...
  const onSave = async (values: any) => {
    try {
      setFormPending(true);
      const res = await upsertProductCategory({
  ...
  const handleToggleActiveClick = (category: CategoryData) => {
  ...
  const handleConfirmToggleActive = async () => {
    ...
    try {
      ...
      const res = await toggleProductCategoryStatus(deactivatingCategory.id, targetState);
  ...
  const renderCell = (item: CategoryData, columnKey: string) => {
    switch (columnKey) {
      case "status":
        return (
          <span
      ...
      default:
        return <span>{String(item[columnKey as keyof CategoryData])}</span>;
    }
  };

  const renderMobileCard = (item: CategoryData) => {
    return (
    ...
    <div className="flex flex-col gap-6">
      ...
      {confirmOpen && deactivatingCategory && (
        <ConfirmDialog
          ...
          } the product category "${deactivatingCategory.name}"?`}
