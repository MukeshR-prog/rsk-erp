...
import { Search, AlertTriangle, CheckCircle, SlidersHorizontal, Settings, X } from "lucide-react";
...
function TradingStockPageContent() {
  ...
  const loadLookupProducts = async () => {
    ...
    if (res.success && res.data) {
      setProductListForLookup(res.data as any[]);
    }
  };
  ...
  }, [page, pageSize, search, selectedTypeFilter]);
  ...
  const renderCell = (item: ProductStockData, columnKey: string) => {
    ...
    switch (columnKey) {
      case "type":
        return (
          <span className={`inline-flex px-2 py-0.5 rounded-lg text-xs font-bold ${
            item.type === "FINISHED_GOOD" ? "bg-blue-50 text-blue-700 dark:bg-blue-950/20 dark:text-blue-400" : "bg-purple-50 text-purple-700 dark:bg-purple-950/20 dark:text-purple-400"
          }`}>
      ...
      case "pieces":
        return <span className="font-extrabold text-emerald-600 dark:text-emerald-400">{totalPieces.toLocaleString()} Pcs</span>;
      case "status":
        return (
      ...
      <div className="flex flex-col gap-2.5 w-full">
        <div className="flex justify-between items-start">
          <div className="flex flex-col">
            ...
            <span className="font-bold text-slate-900 dark:text-slate-50 text-base truncate max-w-[180px]">{item.name}</span>
          </div>
          {isLow ? (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-50 text-red-600 dark:bg-red-950/20 dark:text-red-400">
              <AlertTriangle className="w-3 h-3" />
              <span>Low Stock</span>
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-green-50 text-green-600 dark:bg-green-950/20 dark:text-green-400">
              <CheckCircle className="w-3 h-3" />
              <span>Optimal</span>
            </span>
          )}
        ...
        <div className="grid grid-cols-2 gap-y-2 gap-x-4 border-t border-slate-100 dark:border-slate-850 pt-2.5 mt-1 text-xs font-semibold text-slate-500">
          <div>Type: <span className="font-bold text-slate-850 dark:text-slate-350">{item.type}</span></div>
          ...
          <div className="col-span-2 border-t border-slate-50 dark:border-slate-800 pt-2 flex justify-between items-center text-sm">
            <span className="text-slate-400 text-xs">Total Pieces:</span>
            <span className="font-extrabold text-emerald-600 dark:text-emerald-400">{totalPieces.toLocaleString()} Pcs</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-3.5 sm:gap-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        {/* Type Select filter */}
        <div className="flex items-center gap-2 bg-white dark:bg-slate-950 px-3.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm w-full md:w-auto">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    className={`flex min-h-[70px] w-full rounded-xl border bg-white px-3 py-2 text-sm outline-none transition-all font-semibold dark:bg-slate-955 ${
                      errors.notes ? "border-red-500 focus:border-red-600" : "border-slate-200 focus:border-slate-900 dark:border-slate-800"
                    }`}
                  />
