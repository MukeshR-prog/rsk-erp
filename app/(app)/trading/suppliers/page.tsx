...
function SuppliersPageContent() {
  ...
  const renderCell = (item: ContactData, columnKey: string) => {
    switch (columnKey) {
      case "outstandingBalance":
        return <span className={`font-bold ${item.outstandingBalance > 0 ? "text-red-650" : "text-slate-700"}`}>₹{item.outstandingBalance.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>;
      case "status":
        return (
    ...
    return (
      <div className="flex flex-col gap-2.5 w-full">
        <div className="flex justify-between items-start">
          ...
          <span className={`font-bold text-sm ${item.outstandingBalance > 0 ? "text-red-650" : "text-slate-700"}`}>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-50 dark:border-slate-905/30 pt-2 mt-1">
          <Button size="sm" variant="ghost" onPress={() => router.push(`/trading/suppliers/${item.id}`)} className="border-none min-w-0 p-1 text-slate-650 font-bold">
            <Eye className="w-4 h-4 mr-1 text-slate-650" />
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        {isPending ? (
          <div className="py-20 text-center font-medium text-slate-500">Loading suppliers directory...</div>
        ) : (
          <Table<ContactData>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5 col-span-2">
                  ...
                  <input
                    ...
                    className={`flex h-10 w-full rounded-xl border bg-white px-3 py-2 text-sm outline-none transition-all font-semibold dark:bg-slate-950 ${
                      errors.name ? "border-red-500 focus:border-red-605" : "border-slate-205 focus:border-slate-900 dark:border-slate-800"
                    }`}
                  />
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    className="flex h-10 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-slate-909 dark:border-slate-800 dark:bg-slate-955 outline-none font-semibold transition-all"
                  />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    {...register("state")}
                    className="flex h-10 w-full rounded-xl border border-slate-205 bg-white px-3 py-2 text-sm focus:border-slate-909 dark:border-slate-800 dark:bg-slate-955 outline-none font-semibold transition-all"
                  />
                </div>
